...
public float y0 = -0.7f, sY, firstY = 1f, secondY = -2f, x0 = 0, x; //squareEquation
...
void LetClone (ref GameObject clone, ref float y)
{
		...
		if (clone.rigidbody2D.position.x < 4f) {			//ГЕНЕРИРУЕМ ДО ТОГО КАК ОН БУДЕТ В УКАЗАННОЙ ТОЧКЕ 
			...
			case 1:
				...
				next = values [next];
				if (next == 1) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x+ Random.Range(halfLengthOfGreat,2*halfLengthOfGreat) + 5f * t1, y), Quaternion.identity) as GameObject; //5-скорость платформы
				...
				if (next == 0) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (halfLengthOfGreat, halfLengthOfGreat+halfLengthOfMiddle) + 5f * t1, y), Quaternion.identity) as GameObject;
				...
				if (next == 2) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (halfLengthOfGreat, halfLengthOfGreat+halfLengthOfMini) + 5f * t1, y), Quaternion.identity) as GameObject;
			...
			case 0:
				...
				next = values [next];
				if (next == 1) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (halfLengthOfMiddle, halfLengthOfMiddle+halfLengthOfGreat)  + 5f * t1, y), Quaternion.identity) as GameObject;
					//prev = next;
				...
				if (next == 0) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (halfLengthOfMiddle, 2*halfLengthOfMiddle) + 5f * t1, y ), Quaternion.identity) as GameObject;
					//prev = next;
				}
				if (next == 2) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x +Random.Range (halfLengthOfMiddle,halfLengthOfMiddle+halfLengthOfMini) + 5f * t1, y ), Quaternion.identity) as GameObject;
					//prev = next;
				}
				// TODO: FOR Albert: почему-то ставится гравитация следующей платформы, а не текущей
				prev = next;
			...
			case 2:
				...
				next = values [next];
				if (next == 1) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (halfLengthOfMini, halfLengthOfMini+halfLengthOfGreat) + 5f * t1, y ), Quaternion.identity) as GameObject;
					prev = next;
					//clone1 = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (0f, 0.8f) + 5f * t1, secondY - 0.8f), Quaternion.identity) as GameObject;
				}
				if (next == 0) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (halfLengthOfMini, halfLengthOfMini+halfLengthOfMiddle) + 5f * t1, y ), Quaternion.identity) as GameObject;
					//prev = next;
				}
				if (next == 2) {
					clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x+Random.Range(halfLengthOfMini, 2*halfLengthOfMini) + 5f * t1, y ), Quaternion.identity) as GameObject;
					//prev = next;
				}
				// TODO: FOR Albert: почему-то ставится гравитация следующей платформы, а не текущей
			...
			y = Random.Range (-1f-standOfNull, y0+extremum)-standOfNull;//ВОТ ЕБАНАЯ ОШИБКА!!! В УРАВНЕНИИ НЕТ ОШИБОК! (-1f,y0+extr) y0 может стать тупо меньше первого аргумента
