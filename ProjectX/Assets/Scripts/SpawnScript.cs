		...
		void Update ()
		{
				...
				if (!flag) {
						if (0 == 1)
								...
								t1 = SquareEquationSmall (y0, firstY);
						if (float.IsNaN (t1)) {//должен ли он сюда заходить если условие не выполнилось <4
								t1 = 0.3f; //костыль!!! надо убрать
						}
						...
						if (clone.rigidbody2D.position.x < 4f) {			//ГЕНЕРИРУЕМ ДО ТОГО КАК ОН БУДЕТ В УКАЗАННОЙ ТОЧКЕ 
								...
								case 1:
										...
										next = values [next];
										if (next == 1) {
												clone = Instantiate (obj [next], new Vector2 (clone.rigidbody2D.position.x + Random.Range (2f, 6f) + 5f * t1, firstY - 0.8f), Quaternion.identity) as GameObject;
												prev = next;
								...
								while (Mathf.Abs(firstY-y0)>funRecord) {//БАГ!!!! нужно продумать!
